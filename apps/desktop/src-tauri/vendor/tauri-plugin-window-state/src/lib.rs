...
/// When the window manager reports maximized but the inner size is clearly smaller than every
/// connected monitor (within `SLACK_PX`), treat the window as *not* maximized for geometry
/// persistence. This matches common false positives on GNOME / Wayland tiling paths where
/// `is_maximized()` is set while the window is only partially tiled or freely resized.
fn inner_size_plausibly_fullscreen_on_any_monitor<R: Runtime>(
    window: &Window<R>,
    size: PhysicalSize<u32>,
) -> tauri::Result<bool> {
    const SLACK_PX: u32 = 120;
    for monitor in window.available_monitors()? {
        let mw = monitor.size().width as u32;
        let mh = monitor.size().height as u32;
        if size.width >= mw.saturating_sub(SLACK_PX) && size.height >= mh.saturating_sub(SLACK_PX)
        {
            return Ok(true);
        }
    }
...
impl<R: Runtime> WindowExtInternal for Window<R> {
    fn update_state(&self, state: &mut WindowState, flags: StateFlags) -> tauri::Result<()> {
        let is_maximized_reported = flags
        ...
        let treat_as_maximized_for_geometry = if is_maximized_reported {
            let size = self.inner_size()?;
            inner_size_plausibly_fullscreen_on_any_monitor(self, size)?
        } else {
            false
        };

        if flags.contains(StateFlags::MAXIMIZED) {
            state.maximized = treat_as_maximized_for_geometry;
        }
        ...
        if flags.contains(StateFlags::SIZE) && !treat_as_maximized_for_geometry && !is_minimized
        {
            let size = self.inner_size()?;
        ...
        if flags.contains(StateFlags::POSITION) && !treat_as_maximized_for_geometry && !is_minimized
        {
