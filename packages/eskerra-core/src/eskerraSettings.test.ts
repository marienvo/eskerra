...
describe('parseEskerraSettings', () => {
  ...
  it('rejects invalid structure', () => {
    expect(() => parseEskerraSettings('null')).toThrow(/settings-shared/);
  });
});

describe('buildEskerraSettingsFromForm', () => {
  const emptyR2 = {endpoint: '', bucket: '', accessKeyId: '', secretAccessKey: ''};

  it('omits r2 when all fields empty', () => {
    const r = buildEskerraSettingsFromForm(emptyR2);
  ...
  it('includes r2 when all set', () => {
    const r = buildEskerraSettingsFromForm({...validR2});
    expect(r).toEqual({ok: true, settings: {r2: validR2}});
  });

  it('persists EU jurisdiction', () => {
    const r = buildEskerraSettingsFromForm({...validR2, jurisdiction: 'eu'});
...
describe('serializeEskerraSettings', () => {
