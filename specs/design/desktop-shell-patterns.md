...
- **Multi-column file body:** segments are separated by a blank line, the line **`::today-section::`**, and another blank line (`\n\n::today-section::\n\n`). The first segment is the default column; each subsequent segment matches the next `columns` label in order. Merge/split helpers live in **`splitMergeTodayRowColumns.ts`**.
