...
- **Desktop release bundles:** `npm run desktop:build` (from the repo root) runs `tauri build` for the desktop workspace. On Linux, [`tauri.linux.conf.json`](../../apps/desktop/src-tauri/tauri.linux.conf.json) sets **`bundle.targets` to `["rpm"]` only** (Fedora-oriented shipping; see [`platform-targets.md`](platform-targets.md)). The built RPM is emitted under `apps/desktop/src-tauri/target/release/bundle/rpm/` (exact filename includes version and architecture).
